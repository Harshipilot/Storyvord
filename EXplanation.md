...
2. Django creates a task entry in the database  
3. Celery worker picks up the task asynchronously  
4. AI service (Gemini/OpenAI) processes the request  
5. Result is stored in database  
6. User polls API to retrieve:
...
- Old `ChatCompletion.create()` method deprecated
...
Debugging approach:
- Detailed Celery logs
- Independent API testing with sample payloads
- Structured logging for:
  - API status codes
  - Exception stack traces
- Fallback system to isolate external dependency failures
...
- Redis + Celery chosen over complex event systems (Kafka)
- Keeps system lightweight and easier to deploy

---

### API Dependency vs Reliability
...
- Increased logging improves debugging
- Slight performance overhead accepted for maintainability

---

## Limitations

- No real-time WebSocket updates (polling-based tracking used)
...
- Switch to event-driven architecture for high scale AI systems
